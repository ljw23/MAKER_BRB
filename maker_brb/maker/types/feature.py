...
class Feature(BaseModel):
    ...
    def __str__(self):
        return '_'.join(
            [str(_feature_info) for _feature_info in self.feature_info])
...
class Feature_builder:
            ...
            for feature_info in feature_info_list:
                self.feature_list.append(
                    Feature(feature_id=feature_id,
                            h='all',
                            feature_info=sorted(
                                feature_info, key=lambda x: str(x))))  #无假设纯特征
                self.feature_list.append(
                    Feature(feature_id=feature_id,
                            h=h,
                            feature_info=sorted(feature_info,
                                                key=lambda x: str(x))))
                feature_id += 2

    def find_feature_id(self, h=None,
                        feature_info: List[Feature_info] = None) -> int:
        for feature in self.feature_list:
            if feature_info == feature.feature_info and (not h
                                                         or h == feature.h):
    ...
    def find_v_features(self,
                        v_list: List[str] = None,
                        h: str = None,
                        level: int = 0) -> List[Feature]:
        '''
        根据v特征列表以及层级查询特征
        '''
        ...
        for feature in self.feature_list:
            ...
            if not v_list:
                ...
                continue
            v_list_in_feature = [
                feature_info.v for feature_info in feature.feature_info
            ]
...
if __name__ == '__main__':
    ...
    pprint(
        feature_builder.find_feature_id(
            h='1', feature_info=[Feature_info(v='V1', q='B1')]))
