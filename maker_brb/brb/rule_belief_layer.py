...
class Rule_belief_layer(torch.nn.Module):
    ...
    def reset_parameters(self):
        init.kaiming_uniform_(self.beta, a=math.sqrt(5))  #belief degree随机初始化，可专家规则制定。
        self.beta.data = softmax(self.beta, dim=1)
    
    def forward(self, W_rule_act:Tensor):
        '''
        对论文公式进行了化简，采用p/sum(p)公式代替
        W_rule_act: [num_k, ] 规则激活权重 
        '''
        p_n = torch.empty(self.num_h)
        for n in range(self.num_h):
            product_term1 = 1 # W*beta+1-W
            product_term2 = 1
            for k in range(self.num_k):     
                product_term1 *= W_rule_act[k]*self.beta[k,n] + 1 - W_rule_act[k]
                product_term2 *= 1 - W_rule_act[k]
            
            p_n[n] = product_term1 - product_term2

        p_n = p_n/torch.sum(p_n)
        return p_n
