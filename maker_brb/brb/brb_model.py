...
class BRB_Model(torch.nn.Module):
    ...
    def post_mapping(self):
        self.rule_weight_layer.theta = Parameter(
            map_0_1(self.rule_weight_layer.theta))

        for k in range(self.num_k):
            self.rule_weight_layer.delta_[k] = Parameter(
                map_0_1(self.rule_weight_layer.delta_[k]))
            self.rule_belief_layer.beta[k] = Parameter(
                map_sigma_0_1(self.rule_belief_layer.beta[k]))
